package nl.sajansen.kotlinsnakegame.gui.utils
...
fun scaleImage(image: BufferedImage, width: Int, height: Int): BufferedImage {
    if (image.width == width && image.height == height) {
        return image
    }

    val scaledImage = image.getScaledInstance(width, height, Image.SCALE_FAST)
    val bufferedImage = BufferedImage(width, height, BufferedImage.TYPE_INT_ARGB)
...
fun JSplitPane.divider(): BasicSplitPaneDivider =
    this.components.find { it is BasicSplitPaneDivider } as BasicSplitPaneDivider